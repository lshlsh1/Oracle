...
--> 부모 테이블 → 참조받는 컬럼 → PRIMARY KEY 제약조건을 가지게된다
--  자식 테이블 → 참조하는 컬럼 → FOREIGN KEY 제약조건을 가지게된다
...
-- PRIMARY KEY 제약조건은 반드시 없거나 있거나 단 두가지
-- 그러나 PRIMARY리 KEY를 가지고 있는 컬럼 
-- 싱글 PRIMARY KEY 복합 PRIMARY KEY??
...
-- 이 상황에서 부서명 사원명 직급 급여 을 항상 함께 검색해야하는 경우에 사원테이블에 부서명을 추가하자
...
+++++++  =======
 (P.K)   (F.K)
--------------------------------------------------------
9XXXXXXX  7369   아내 이연희 
0XXXXXXX  7369   아들 강동원
9XXXXXXX  7370   아내 아이유 
9XXXXXXX  7371   남편 박형식 
                  :
- 주민번호를 식별자로.... 중복된 값X, 고유번호

- 사원테이블의 직급, 사원가족 테이블의 관계 컬럼이 잘못 설계되어있음
관계는 관계가 아닌 관계코드 컬럼이어야함. (1,2,3...)
그리고 관계테이블, 직급테이블을 따로 만들어야함.
