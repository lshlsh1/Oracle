...
SELECT NAME"이름", SSN"주민번호"
FROM TBL_INSA
WHERE SUBSTR(SSN,1,2) >=70 AND SUBSTR(SSN,1,2) <80
  AND CITY = '서울';
...
SELECT T.생년월일, COUNT(*)"몇명"
FROM
(
SELECT SUBSTR(SSN,1,2)+1900"생년월일"
FROM TBL_INSA
)T
GROUP BY T.생년월일;
...
SELECT CASE GROUPING(T.인원수) WHEN 1 THEN '전체인원수' ELSE TO_CHAR(T.인원수) END"조회", COUNT(*)
FROM
(
...
FROM
(
SELECT DECODE(SUBSTR(SSN,8,1),'1','남자','여자')"인원수"
FROM TBL_INSA
WHERE BUSEO='영업부'
)T
GROUP BY T.인원수;
...
SELECT T.인원수, SUM(T.기본급)
FROM
(
SELECT DECODE(SUBSTR(SSN,8,1),'1','남자','여자')"인원수", BASICPAY"기본급"
FROM TBL_INSA
)T
GROUP BY T.인원수;

--43. 남자와 여자의 기본급 평균값 조회. AVG(), DECODE() 함수 이용.
SELECT T.인원수, AVG(T.기본급)
FROM
(
SELECT DECODE(SUBSTR(SSN,8,1),'1','남자','여자')"인원수", BASICPAY"기본급"
FROM TBL_INSA
...
SELECT T.부서, T.인원수, COUNT(*)
FROM
(
SELECT DECODE(SUBSTR(SSN,8,1),'1','남자','여자')"인원수", BUSEO"부서"
FROM TBL_INSA
)T
GROUP BY T.부서, T.인원수
ORDER BY T.부서, T.인원수;
...
SELECT T.지역, T.인원수, COUNT(*)
FROM
(
SELECT DECODE(SUBSTR(SSN,8,1),'1','남자','여자')"인원수", CITY"지역"
FROM TBL_INSA
)T
GROUP BY T.지역, T.인원수
ORDER BY T.지역, T.인원수;
...
GROUP BY T.입사년도, T.인원수
ORDER BY T.입사년도, T.인원수;
...
SELECT DECODE(SUBSTR(SSN,8,1),'1','남자','여자')"인원수"
     , EXTRACT(YEAR FROM IBSADATE)"입사년도"
FROM TBL_INSA
WHERE BUSEO ='영업부' AND BUSEO='총무부'
)T
GROUP BY T.입사년도, T.인원수
ORDER BY T.입사년도, T.인원수;

--49. 서울 사람중 부서별 남자와 여자인원수, 남자와 여자 급여합 조회.;
SELECT T.부서, T.인원수, COUNT(*), SUM(T.급여)
FROM
(
SELECT DECODE(SUBSTR(SSN,8,1),'1','남자','여자')"인원수", BUSEO"부서" , BASICPAY+SUDANG"급여"
FROM TBL_INSA
WHERE CITY = '서울' 
)T
GROUP BY T.부서, T.인원수
...
SELECT T.부서, COUNT(T.남자), COUNT(T.여자)
FROM
(
SELECT BUSEO"부서"
, DECODE(SUBSTR(SSN,8,1),'1','남자','3','남자')"남자"
, DECODE(SUBSTR(SSN,8,1),'2','여자','4','여자')"여자"
FROM TBL_INSA
)T
GROUP BY T.부서
HAVING COUNT(T.여자)>=5;
...
--    나이: 주민번호 활용
SELECT NAME"이름", DECODE(SUBSTR(SSN,8,1),'1','남자','3','남자','여자')"성별"
     , EXTRACT(YEAR FROM SYSDATE)-SUBSTR(SSN,1,2)-1899"나이"
FROM TBL_INSA;

--53. 서울 사람 중에서 기본급이 200만원 이상인 사람 조회. 
--    ( 이름, 기본급 );
SELECT NAME"이름", BASICPAY
FROM TBL_INSA
...
--    1월    10명
--    2월    25명
SELECT T.월, COUNT(*)"인원수"
FROM
(
SELECT TO_CHAR(IBSADATE,'M')||'월'"월"
FROM TBL_INSA
)T
GROUP BY T.월
ORDER BY T.월;
...
FROM TBL_INSA
--WHERE IBSADATE<'2009-04-04';
--WHERE TO_CHAR(IBSADATE,'YYYY-MM-DD')<'2009-04-04';
...
FROM TBL_INSA
WHERE TO_NUMBER(SUBSTR(SSN,1,2)) BETWEEN 75 AND 85;
...
SELECT NAME, SSN, TO_CHAR(IBSADATE,'YYYY-MM-DD DAY')"입사일"
FROM TBL_INSA
WHERE SUBSTR(SSN,8,1)IN('1','3');
