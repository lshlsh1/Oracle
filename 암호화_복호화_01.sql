...
--3. 이 패키지는 4개의 프로시저로 이루어져있다.
--   VARCHAR2 타입을 Encrypt/Decrypt 할 수 있는 2개의 프로시저
--   RAW 타입을 Encrypt/Decrypt 할 수 있는 2개의 프로시저
--   (다른 타입은 지원하지 않기 때문에 NUMBER인 경우는 TO_CHAR() 이용)

-- ※ RAW, LONG RAW, ROWID 타입
--    그래픽 이미지나 디지털 사운드 등을 저장
--    HEXA-DECIMAL(16진수) 형태로 RETURN
--    이 중 RAW는 VARCHAR2와 유사하다
--    LONG RAW는 LONG과 유사하지만 다음과 같은 제한사항이 있다.
--    ·저장과 추출만 가능하고, DATA를 가공할 수 없다
--    ·LONG RAW는 LONG과 같은 제한사항을 갖는다.

--○ 해당 패키지를 사용할 수 있도록 권한 부여
GRANT EXECUTE ON DBMS_OBFUSCATION_TOOLKIT TO SCOTT;
--==>> Grant을(를) 성공했습니다.
--> SCOTT 계정으로 DBMS_OBFUSCATION_TOOLKIT 패키지의 프로시저를 사용할 수 있는 권한 부여

GRANT EXECUTE ON DBMS_OBFUSCATION_TOOLKIT TO PUBLIC;
--==>>Grant을(를) 성공했습니다.
--> PUBLIC 으로 DBMS_OBFUSCATION_TOOLKIT 패키지의 프로시저를 사용할 수 있는 권한 부여
